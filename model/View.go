...
func (v View) GetContent(data ViewInterface, scope string, session *h.Session, ctx *fasthttp.RequestCtx) string {
	...
	if validContent && content != nil {
		...
		h.PrintlnIf("has content in cache, not expired, return content", h.GetConfig().Mode.Debug && h.CacheLog)
		...
		if h.CacheStorage.CacheInProgress(v.template, v.cacheKeys) {
			...
			h.PrintlnIf("has content in cache, expired, cache process in progress, return content", h.GetConfig().Mode.Debug && h.CacheLog)
		} else {
			//hasn't started yet the processing
			h.PrintlnIf("has content in cache, expired, start cache process, return content", h.GetConfig().Mode.Debug && h.CacheLog)
		...
		//no content
		if h.CacheStorage.CacheInProgress(v.template, v.cacheKeys) {
			h.PrintlnIf("no content, cache in progress, ticking for cache is ready", h.GetConfig().Mode.Debug && h.CacheLog)
			...
			h.PrintlnIf("no content, cache is not in progress", h.GetConfig().Mode.Debug && h.CacheLog)
			v.cache(data, scope, session, ctx)
		}
