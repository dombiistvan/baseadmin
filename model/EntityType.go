...
func (et EntityType) BuildStructure(dbmap *gorp.DbMap) {
	...
	if Conf.Mode.Rebuild_structure {
		h.PrintlnIf(fmt.Sprintf("Drop %s table", et.GetTable()), Conf.Mode.Rebuild_structure)
		dbmap.Exec(fmt.Sprintf("DROP TABLE IF EXISTS %s;", et.GetTable()))
	}

	h.PrintlnIf(fmt.Sprintf("Create %s table", et.GetTable()), Conf.Mode.Rebuild_structure)
